...
public class GoogleController  {
	...
	final static String GOOGLE_CALL_BACK_URL = "http://localhost:8080/app/auth";
	...
	public String googleRegister(Model model, @RequestParam(value = "code") String authCode, HttpSession session, HttpServletResponse response)
		...
		GoogleOAuthRequest  googleOAuthRequestParam = GoogleOAuthRequest
				...
				.clientSecret(clientSecret)
				.code(authCode)
				.redirectUri("http://localhost:8080/app/socialRegister")
				.grantType("authorization_code").build();
		...
		if (member == null) {			
			...
			PrintWriter out = response.getWriter(); out.println("<script>alert('해당 아이디가 있습니다.'); location.href='./login';</script>"); 
			out.flush();
